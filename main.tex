...
\be
    F_{B1}(\mathbf{x}) = \int \mathrm{d}r\, \mathrm{d}\theta\, \mathrm{d}\phi\, r^2 \sin{\theta} \, R(r)\frac{\delta(\theta)}{\sin{\theta}} \frac{\delta\left(\left|\mathbf{r} - \mathbf{x}\right| - \Delta\right)}{\left|\mathbf{r} - \mathbf{x}\right|}
\ee
...
\be
    F_{B1}(\mathbf{x}) = \int \mathrm{d}\theta\, \mathrm{d}\phi\, \mathrm{d}r\, r^2 \, R(r) \delta(\theta) \frac{\delta\left(\sqrt{r^2 + x^2 - 2rx \cos\gamma} - \Delta\right)}{\sqrt{r^2 + x^2 - 2rx \cos\gamma}}
\ee
...
\begin{center}
  ...
  \end{tabular}
\end{center}
Then we do another simplification, that is $\gamma = 0, \pi$, i.e., align $\mathbf{x}$ on the axis of $\mathbf{r}$. The reason we do this simplification is that we wonder how should Bob measure the field on the axis where also should has the most significant non-isotropic phenonenon.
\subsection{Result}
